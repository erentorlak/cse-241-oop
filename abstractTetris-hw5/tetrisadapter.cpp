...
template <typename T>
TetrisAdapter<T>::TetrisAdapter(int width_inp, int height_inp)
{
    height = height_inp;
    width = width_inp;
    totalMoves = 0;
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::fill_dots(){
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::draw() const {
...
template <typename T>
char TetrisAdapter<T>::TetrisAdapter::move_direction(){
...
template <typename T>
int TetrisAdapter<T>::TetrisAdapter::move_count(Tetromino const tetro, char move_direction){     //gets input of right or left move 
                                                                    //and checks validity
    int inp_count;
    while(1) {
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::operator+=(const Tetromino& tetro) {    //adds tetromino to the up-middle of board     
...
template <typename T>
int TetrisAdapter<T>::TetrisAdapter::can_moveDown(const Tetromino tetro) {
...
template <typename T>
int TetrisAdapter<T>::TetrisAdapter::lastMove(const Tetromino tetro){
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::move_right(Tetromino tetro, int move_slide){
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::move_left(Tetromino tetro, int move_slide){
    for (int k = 0; k < move_slide; k++){
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::animate(Tetromino tetro, char move_direction, int move_slide) {
    int down;
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::writeToFile(const string& fileName) {
...
template <typename T>
void TetrisAdapter<T>::TetrisAdapter::readFromFile(const string& fileName)  {
    ...
    while(getline(f_read, line)) {
        ...
        using row_type = typename T::value_type;
        row_type temp;
        for (char c : line) {
            temp.emplace_back(element_type{c});
        }
        vect.emplace_back(std::move(temp));
    }
    f_read.close();   
    ...
    // board = vect;  //  This line can make playing the game from saved text file
